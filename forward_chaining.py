...
class InferenceEngine:
    """
    Class to handle the inference process using forward chaining.
    """
    ...
    def forward_chain(self, facts, rule_sets):
        """
        Apply the forward chaining process to infer facts.
        ...
        while new_inferred_facts:
            ...
            for set_name, rule_set in rule_sets.items():
                for rule in rule_set['rules']:
                    ...
                    if self.evaluate_premise(inferred_facts, premise):
                        fact_name, fact_value = conclusion.split('=')
                        fact_name = fact_name.strip()
                        fact_value = int(fact_value.strip())
                        
                        if fact_name not in inferred_facts:
                            inferred_facts[fact_name] = fact_value
                            new_inferred_facts = True
                            # print(f"Inferred fact: {fact_name} = {fact_value}")
        
        return inferred_facts

class Predictor:
    """
    Class to handle the prediction process using the given rules utilsig forward chaining.
    """
    ...
    def predict(self):
        ...
        This fucntion will iteratively apply the defined rules against the facts to determine if an infered rule exists, and then used to generate a prediction
        It will use iterate through all the sets defined in the Yaml configuration file for each Set of rules that are defined.
        ...
        for set_name, rule_set in self.rules['rule_sets'].items():
            ...
            for _, row in self.data.iterrows():
                facts = row.to_dict()
                final_facts = self.inference_engine.forward_chain(facts, {set_name: rule_set})
